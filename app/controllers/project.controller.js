...
//Update a project capital expenditure with id
exports.updateCapExp = (req, res) => {
  const id = req.params.id;
  ...
  Project.update(req.body, { 
    ...
    if (num == 1 ) {
      res.send({
        message: "Project capexp what updated successfully.",
      });
    }else{
      res.send({
        message: `Cannot update Project with id=${id}. Project was not found req.body is empty!`,
      });
    }
  })
  .catch((err) => {
